...
basetabfunc_inner <- function(x, data, stratavar, catdig, contdig) {
  ...
  if (cl %in% c("character", "factor")) {
    ...
    out <- tmp %>%
      ...
      mutate(
        out = paste0(fn(med, contdig), " [", fn(min, contdig), ", ", fn(q1, contdig), ", ", fn(q3, contdig), ", ", fn(max, contdig), "], ", fn(mean, contdig), " (", fn(sd, contdig), ")"),
        out = case_when(
          n == 0 ~ "-",
          n < 10 ~ "<10",
          TRUE ~ out
...
basetabfunc <- function(data, novars = NULL, stratavar) {
  tab <- data %>%
...
#| tbl-cap: "Baseline characteristics by patients status"
#| tbl-pos: "H"

basetabfunc(data = rsdata, stratavar = "sos_prevhfh3mo", novars = c("sos_prevhfh3mo"))
```

```{r}
#| label: tbl-base-areahosp
#| cache: true
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics by area of previous hospitalization (only for patients with a previous HFH withn 3 months)"
#| tbl-pos: "H"

basetabfunc(data = rsdata %>% filter(sos_prevhfh3mo == "Yes"), stratavar = "sos_hospward_3mo", novars = c("sos_prevhfh1yr", "sos_hospward3mo", "sos_prevhfh3mo"))
```
